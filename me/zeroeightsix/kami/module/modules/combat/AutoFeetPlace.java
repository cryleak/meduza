...
extends Module {
    private final Vec3d[] surroundTargets = new Vec3d[]{new Vec3d(0.0, 0.0, 0.0), new Vec3d(1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, 1.0), new Vec3d(-1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, -1.0), new Vec3d(1.0, 0.0, 0.0), new Vec3d(0.0, 0.0, 1.0), new Vec3d(-1.0, 0.0, 0.0), new Vec3d(0.0, 0.0, -1.0), new Vec3d(1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, 1.0), new Vec3d(-1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, -1.0)};
    private final Vec3d[] surroundTargetsCritical = new Vec3d[]{new Vec3d(0.0, 0.0, 0.0), new Vec3d(1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, 1.0), new Vec3d(-1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, -1.0)};
    ...
    private Setting<Double> blockPerTick = this.register(Settings.d("Blocks per Tick", 4.0));
    ...
    public void onUpdate() {
        if (this.isDisabled() || AutoFeetPlace.mc.field_71439_g == null || ModuleManager.isModuleEnabled("Freecam")) {
            return;
        }
        if (this.offsetStep == 0) {
            this.basePos = new BlockPos(AutoFeetPlace.mc.field_71439_g.func_174791_d()).func_177977_b();
    ...
    protected void onDisable() {
        ...
        if (this.lastHotbarSlot != this.playerHotbarSlot && this.playerHotbarSlot != -1) {
            ...
            if (this.spoofHotbar.getValue().booleanValue()) {
                AutoFeetPlace.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketHeldItemChange(this.playerHotbarSlot));
            } else {
    ...
    public void placeBlockExecute(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!AutoFeetPlace.canBeClicked(neighbor)) {
            ...
            if (this.spoofRotations.getValue().booleanValue()) {
                AutoFeetPlace.faceVectorPacketInstant(hitVec);
            }
            boolean needSneak = false;
            Block blockBelow = AutoFeetPlace.mc.field_71441_e.func_180495_p(neighbor).func_177230_c();
            ...
            if (this.lastHotbarSlot != obiSlot) {
                ...
                if (this.spoofHotbar.getValue().booleanValue()) {
                    AutoFeetPlace.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketHeldItemChange(obiSlot));
                } else {
                    Wrapper.getPlayer().field_71071_by.field_70461_c = obiSlot;
                }
                this.lastHotbarSlot = obiSlot;
            }
            AutoFeetPlace.mc.field_71442_b.func_187099_a(Wrapper.getPlayer(), AutoFeetPlace.mc.field_71441_e, neighbor, side2, hitVec, EnumHand.MAIN_HAND);
            AutoFeetPlace.mc.field_71439_g.field_71174_a.func_147297_a((Packet)new CPacketAnimation(EnumHand.MAIN_HAND));
            if (needSneak) {
                if (this.debugMessages.getValue().booleanValue()) {
    ...
    private static boolean canBeClicked(BlockPos pos) {
        return AutoFeetPlace.getBlock(pos).func_176209_a(AutoFeetPlace.getState(pos), false);
    }

    private static Block getBlock(BlockPos pos) {
        return AutoFeetPlace.getState(pos).func_177230_c();
    }
    ...
    private static void faceVectorPacketInstant(Vec3d vec) {
        float[] rotations = AutoFeetPlace.getLegitRotations(vec);
    ...
    private static float[] getLegitRotations(Vec3d vec) {
        Vec3d eyesPos = AutoFeetPlace.getEyesPos();
