    ...
    private Event parseNode(boolean block, boolean indentlessSequence) {
        ...
        if (indentlessSequence) {
            if (this.scanner.checkToken(Token.ID.BlockEntry)) {
                ...
                SequenceStartEvent sequenceStartEvent = new SequenceStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.FALSE);
        ...
        if (this.scanner.checkToken(Token.ID.Scalar)) {
            ...
            ScalarEvent scalarEvent = new ScalarEvent(anchor, tag, implicitValues, token.getValue(), startMark, endMark, Character.valueOf(token.getStyle()));
            this.state = this.states.pop();
            return var3_17;
        }
        if (this.scanner.checkToken(Token.ID.FlowSequenceStart)) {
            endMark = this.scanner.peekToken().getEndMark();
            SequenceStartEvent sequenceStartEvent = new SequenceStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.TRUE);
            this.state = new ParseFlowSequenceFirstEntry();
            return var3_17;
        }
        if (this.scanner.checkToken(Token.ID.FlowMappingStart)) {
            endMark = this.scanner.peekToken().getEndMark();
            MappingStartEvent mappingStartEvent = new MappingStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.TRUE);
        ...
        if (block) {
            if (this.scanner.checkToken(Token.ID.BlockMappingStart)) {
                endMark = this.scanner.peekToken().getStartMark();
                MappingStartEvent mappingStartEvent = new MappingStartEvent(anchor, tag, implicit, startMark, endMark, Boolean.FALSE);
                this.state = new ParseBlockMappingFirstKey();
                return var3_17;
            }
        }
        if (anchor != null || tag != null) {
            ScalarEvent scalarEvent = new ScalarEvent(anchor, tag, new ImplicitTuple(implicit, false), "", startMark, endMark, Character.valueOf('\u0000'));
            this.state = this.states.pop();
            return var3_17;
        }
